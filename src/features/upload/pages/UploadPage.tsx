...
export default function UploadPage() {
  ...
  const errorCount = uploadJobs.filter(
    (q) => q.status === 'error' || q.status === 'interrupted',
  ).length;
  const totalProgress =
    uploadJobs.length > 0
      ? uploadJobs.reduce((sum, q) => sum + (q.status === 'complete' ? 100 : q.progress), 0) /
        uploadJobs.length
      : 0;
                ...
                <span className="text-sm font-normal text-muted-foreground ml-2">
                  ...
                  {errorCount > 0 && (
                    <span className="text-destructive ml-1">
                      {t('upload.failed', { count: errorCount })}
                    </span>
                  )}
                </span>
              </CardTitle>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2 min-w-[120px]">
                        ...
                        <span className="text-xs text-muted-foreground w-10 text-right">
                          {Math.round(item.status === 'complete' ? 100 : item.progress)}%
                        </span>
                      </div>
                    </TableCell>
                    <TableCell>

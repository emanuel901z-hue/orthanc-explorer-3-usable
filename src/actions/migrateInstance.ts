/**
 * migrateInstanceAction — audit-seam wrapper for migrating an instance into a study.
 *
 * Side effects:
 *   1. Calls studiesApi.merge(targetStudyId, [instanceId], keepSource).
...
export async function migrateInstanceAction(
  targetStudyId: string,
  instanceId: string,
  keepSource = false,
): Promise<OrthancMergeResult> {
  const base = makeAuditBase('instance.migrate', 'instance', instanceId);
  auditClient.emit({ ...base, outcome: 'started', detail: { targetStudyId, keepSource } });
  try {
    const result = await studiesApi.merge(targetStudyId, [instanceId], keepSource);
    auditClient.emit({
      ...base,
      outcome: 'success',
      detail: {
